...
VERSION_TAG="vNext.7.4.21"

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
MANIFEST="$ROOT/kalman-hub-recovery/v7.4.21/source_manifest.ndjson"
STAMP="$(date +%Y%m%d_%H%M%S)"
WORK="/tmp/kalman-hub-v7421-${STAMP}"
SRC="$WORK/source"
mkdir -p "$SRC"
...
echo "[1/6] Decode audited v7.4.21 source"
...
grep -F "AUTO TRADE · NEXT ACTION" "$SRC/index.html" >/dev/null
...
assert "AUTO TRADE · NEXT ACTION" in index
assert "LIVE_CANARY_TARGET_KRW=5000" in app
assert "R5.1 SHADOW CANARY" in app
...
assert account.get("status") == "READY"
assert account.get("trade_execution") is False
print("[PASS] production v7.4.21 visible and web remains read-only")
PY
