...
VERSION_TAG="vNext.7.4.29"

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
BUILDER="$ROOT/kalman-toss-gateway/scripts/build_investment_hub_v7429_neon_selectors.py"
MANIFEST="$ROOT/kalman-hub-recovery/v7.4.29/source_manifest.ndjson"
STAMP="$(date +%Y%m%d_%H%M%S)"
WORK="/tmp/kalman-hub-v7429-${STAMP}"
SRC="$WORK/source"
mkdir -p "$SRC"
...
echo "[1/7] Build audited v7.4.29 source"
python3 "$BUILDER"
...
print("[PASS] production v7.4.29 visible; US/KR selectors use canonical Neon snapshots; R5.1 ledger preserved; web remains read-only")
PY
