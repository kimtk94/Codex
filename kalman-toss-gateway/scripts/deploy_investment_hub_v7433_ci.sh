...
VERSION_TAG="vNext.7.4.33"

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
BUILDER="$ROOT/kalman-toss-gateway/scripts/build_investment_hub_v7433_shadow_ranking_neon.py"
MANIFEST="$ROOT/kalman-hub-recovery/v7.4.33/source_manifest.ndjson"
STAMP="$(date +%Y%m%d_%H%M%S)"
WORK="/tmp/kalman-hub-v7433-${STAMP}"
SRC="$WORK/source"
mkdir -p "$SRC"
...
echo "[1/7] Build audited v7.4.33 source"
python3 "$BUILDER"
...
assert str(shadow.get("ranking_code_sha") or "").startswith("b917778"),shadow
...
print("[PASS] production v7.4.33 visible; SHADOW signals and A/B/C ranking read from Neon; existing read models preserved; web remains read-only")
PY
