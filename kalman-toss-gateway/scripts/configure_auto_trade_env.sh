...
if [ "$PROFILE" = "live-canary-5000" ]; then
  printf '%s\n' 'LIVE enabled: R5.1_BASE_HGB / R5_LIVE_TOP1 / KRW 5,000 per entry / up to 3 entries per symbol (KRW 15,000 target cap) / max 3 active managed positions / no fixed daily buy cap. Research non-overlap remains benchmark-only.'
else
