...
VERSION_TAG="vNext.7.4.34"

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
BUILDER="$ROOT/kalman-toss-gateway/scripts/build_investment_hub_v7434_market_sites.py"
MANIFEST="$ROOT/kalman-hub-recovery/v7.4.34/source_manifest.ndjson"
STAMP="$(date +%Y%m%d_%H%M%S)"
WORK="/tmp/kalman-hub-v7434-${STAMP}"
SRC="$WORK/source"
mkdir -p "$SRC"
...
echo "[1/7] Build audited v7.4.34 source"
python3 "$BUILDER"
...
print("[PASS] production v7.4.34 visible; SHADOW signals and A/B/C ranking read from Neon; existing read models preserved; web remains read-only")
PY
