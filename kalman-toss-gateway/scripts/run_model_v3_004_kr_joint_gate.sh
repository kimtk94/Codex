...
MODEL_SPEC="${KALMAN_MODEL_V3004_KR_SPEC:-$APP_ROOT/config/model-v3-004-kr-joint-gate-spec.json}"
...
ROOT="${KALMAN_MODEL_V3004_KR_ROOT:-$DATA_ROOT/Market_Model_V3_004_KR_JointGate}"
MATRIX_DIR="$ROOT/matrices"
MODEL_DIR="$ROOT/frozen"
REPORT="$MODEL_DIR/latest.json"

cd "$APP_ROOT"
exec 9>"$LOCK_DIR/model-v3-004-kr.lock"
flock -n 9 || { echo "MODEL_V3_004_KR_ALREADY_RUNNING" >&2; exit 30; }

echo "=================================================="
echo "KALMAN MODEL V3.004 KR JOINT-GATE"
echo "=================================================="
echo "Model root     : $ROOT"
echo "Forward start  : 2026-09-23"
echo "Fixed candidate: f8_c0.001_cwnone"
echo "Top fraction   : 0.20"
...
echo
echo "[2/3] Build isolated KR V3.004 matrix"
mkdir -p "$MATRIX_DIR"
...
echo
echo "[3/3] Freeze-or-track KR V3.004 candidate"
mkdir -p "$MODEL_DIR"
"$RESEARCH_PY" -m research.model_v3.kr_joint_gate_candidate   --matrix "$MATRIX_DIR/kr_matrix.parquet"   --matrix-manifest "$MATRIX_DIR/kr_matrix_manifest.json"   --model-spec "$MODEL_SPEC"   --evaluation-spec "$EVAL_SPEC"   --output-dir "$MODEL_DIR"

echo
echo "=================================================="
echo "MODEL_V3_004_KR_COMPLETE"
echo "report=$REPORT"
