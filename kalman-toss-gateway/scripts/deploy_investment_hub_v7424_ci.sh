...
VERSION_TAG="vNext.7.4.24"

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
BUILDER="$ROOT/kalman-toss-gateway/scripts/build_investment_hub_v7424_execution_telemetry.py"
MANIFEST="$ROOT/kalman-hub-recovery/v7.4.24/source_manifest.ndjson"
STAMP="$(date +%Y%m%d_%H%M%S)"
WORK="/tmp/kalman-hub-v7424-${STAMP}"
SRC="$WORK/source"
mkdir -p "$SRC"
...
echo "[1/7] Build audited v7.4.24 source"
python3 "$BUILDER"
...
grep -F "round_trip_cost_bps" "$SRC/api/assets.js" >/dev/null
...
if rows:
    row=rows[0]
    for key in (
        "net_return","round_trip_cost_bps","entry_slippage_bps","exit_slippage_bps",
        "entry_fill_latency_ms","exit_fill_latency_ms"
    ):
        assert key in row,key
print("[PASS] candidate APIs/UI and read-only invariant")
PY
...
assert health.get("account_trade_execution") is False
assert control.get("schema_version")=="kalman-web-control-v2"
print("[PASS] production v7.4.24 visible and web remains read-only")
PY
