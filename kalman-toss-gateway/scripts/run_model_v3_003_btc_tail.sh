...
MODEL_SPEC="${KALMAN_MODEL_V3003_BTC_SPEC:-$APP_ROOT/config/model-v3-btc-tail-spec.json}"
...
ROOT="${KALMAN_MODEL_V3003_BTC_ROOT:-$DATA_ROOT/Market_Model_V3_003_BTC_Tail}"
MATRIX_DIR="$ROOT/matrices"
MODEL_DIR="$ROOT/frozen"
REPORT="$MODEL_DIR/latest.json"

cd "$APP_ROOT"
exec 9>"$LOCK_DIR/model-v3-003-btc-tail.lock"
flock -n 9 || { echo "MODEL_V3_003_BTC_ALREADY_RUNNING" >&2; exit 30; }

echo "=================================================="
echo "KALMAN MODEL V3.003 BTC TAIL-ENTRY"
echo "=================================================="
echo "Model root     : $ROOT"
echo "Forward start  : 2026-09-23"
echo "Tail fractions : 0.05 / 0.10 / 0.15"
echo "Horizon        : 7 observations"
...
echo
echo "[2/3] Build isolated BTC V3 matrix"
mkdir -p "$MATRIX_DIR"
...
echo
echo "[3/3] Audit/freeze BTC tail candidate"
mkdir -p "$MODEL_DIR"
"$RESEARCH_PY" -m research.model_v3.btc_tail_candidate   --matrix "$MATRIX_DIR/btc_matrix.parquet"   --matrix-manifest "$MATRIX_DIR/btc_matrix_manifest.json"   --model-spec "$MODEL_SPEC"   --evaluation-spec "$EVAL_SPEC"   --output-dir "$MODEL_DIR"

echo
echo "=================================================="
echo "MODEL_V3_003_BTC_COMPLETE"
echo "report=$REPORT"
