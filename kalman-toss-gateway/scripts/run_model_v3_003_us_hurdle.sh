...
MODEL_SPEC="${KALMAN_MODEL_V3003_US_SPEC:-$APP_ROOT/config/model-v3-us-hurdle-spec.json}"
...
ROOT="${KALMAN_MODEL_V3003_US_ROOT:-$DATA_ROOT/Market_Model_V3_003_US_Hurdle}"
MATRIX_DIR="$ROOT/matrices"
MODEL_DIR="$ROOT/frozen"
REPORT="$MODEL_DIR/latest.json"

cd "$APP_ROOT"
exec 9>"$LOCK_DIR/model-v3-003-us-hurdle.lock"
flock -n 9 || { echo "MODEL_V3_003_US_ALREADY_RUNNING" >&2; exit 30; }

echo "=================================================="
echo "KALMAN MODEL V3.003 US HURDLE TARGET"
echo "=================================================="
echo "Model root     : $ROOT"
echo "Forward start  : 2026-09-23"
echo "Target hurdles : 0.25% / 0.50% / 1.00%"
echo "Horizon        : 5 observations"
...
echo
echo "[2/3] Build isolated US V3 matrix"
mkdir -p "$MATRIX_DIR"
...
echo
echo "[3/3] Audit/freeze US hurdle candidate"
mkdir -p "$MODEL_DIR"
"$RESEARCH_PY" -m research.model_v3.us_hurdle_candidate   --matrix "$MATRIX_DIR/us_matrix.parquet"   --matrix-manifest "$MATRIX_DIR/us_matrix_manifest.json"   --model-spec "$MODEL_SPEC"   --evaluation-spec "$EVAL_SPEC"   --output-dir "$MODEL_DIR"

echo
echo "=================================================="
echo "MODEL_V3_003_US_COMPLETE"
echo "report=$REPORT"
