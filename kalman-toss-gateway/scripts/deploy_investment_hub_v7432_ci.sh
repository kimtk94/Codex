...
VERSION_TAG="vNext.7.4.32"

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
BUILDER="$ROOT/kalman-toss-gateway/scripts/build_investment_hub_v7432_shadow_neon.py"
MANIFEST="$ROOT/kalman-hub-recovery/v7.4.32/source_manifest.ndjson"
STAMP="$(date +%Y%m%d_%H%M%S)"
WORK="/tmp/kalman-hub-v7432-${STAMP}"
SRC="$WORK/source"
mkdir -p "$SRC"
...
echo "[1/7] Build audited v7.4.32 source"
python3 "$BUILDER"
...
assert shadow.get("signal_source_fresh") is True,shadow
...
assert shadow.get("ranking_source") in ("LEGACY_STANDALONE_BAKEOFF","UNAVAILABLE")
if shadow.get("ranking_source")=="LEGACY_STANDALONE_BAKEOFF":
...
assert shadow.get("signal_source")=="NEON_STRATEGY_SIGNAL",shadow
assert shadow.get("signal_source_fresh") is True
signals=shadow.get("signals") or {}
...
print("[PASS] production v7.4.32 visible; SHADOW signals read from Neon; legacy ranking labeled stale; existing read models preserved; web remains read-only")
PY
