...
if [ "$(id -u)" -ne 0 ]; then
  ...
  echo "  sudo bash kalman-toss-gateway/scripts/deploy_macro_event_v18.sh" >&2
  exit 1
fi
...
BACKUP_DIR="$STATE_DIR/macro-event-v18-backup-$STAMP"
...
echo "[2/6] Backup current runtime macro files"
for rel in "${FILES[@]}"; do
...
echo "[3/6] Install macro v1.8 runtime files"
...
echo "[5/6] Install dedicated cron"
"$APP_ROOT/scripts/install_macro_event_cron_v1.sh"

echo "[6/6] Verify v1.8 config + scheduler"
"$PY" - <<'PY'
import json
from pathlib import Path
p=Path("/opt/kalman/app/config/macro-event-features-v1.json")
cfg=json.loads(p.read_text())
assert cfg["version"] == "macro-event-feature-v1.8.0", cfg.get("version")
impact=cfg["target_market_impact"]
assert impact["targets"]["US"]["indicators"]
assert impact["targets"]["KR"]["indicators"]
assert impact["semantics"] == "ordinal_research_prior_not_causal_effect_size"
print("[PASS] macro-event-feature-v1.8.0 target-market registry")
PY
