...
for ((i=0; i<${#args[@]}; i++)); do
  case "${args[$i]}" in
    --output-dir)
      if (( i + 1 < ${#args[@]} )); then OUTPUT_OVERRIDE="${args[$((i + 1))]}"; fi
      ;;
    --output-dir=*) OUTPUT_OVERRIDE="${args[$i]#--output-dir=}" ;;
  esac
done
...
print(v.get("KALMAN_FINVIZ_OUTPUT_DIR") or f"{root}/Market_Screeners/finviz")
PY
)"
fi
...
cd "$APP_ROOT"
exec flock -n "$LOCK_DIR/finviz-v2.lock" "$PY" -m engine.screeners.cli "$@"
