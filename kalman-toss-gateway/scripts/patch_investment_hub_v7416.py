...
US_VIEW=r'''function renderUS(j){
  ...
  return '<div class="us-workspace"><nav class="market-subnav"><button class="market-subtab active" data-us-view="overview">Overview</button><button class="market-subtab" data-us-view="performance">Performance</button><button class="market-subtab" data-us-view="ledger">Ledger</button><button class="market-subtab" data-us-view="model">Model</button></nav>'+
    '<section class="us-pane active" data-us-pane="overview"><div class="summary">'+staleBadge(j.effective_stale)+' '+badge(s.market_risk||'US')+' '+badge(s.primary_lineage||j.model_version||'MODEL')+' <span class="muted">'+time(j.data_as_of)+'</span></div><div class="us-overview-grid"><div class="card us-primary"><div class="section-title"><div><div class="eyebrow">PRIMARY WORKSPACE</div><h3>Top-ranked price & signals</h3></div><span class="small">first chart = primary</span></div>'+perf+'</div><div class="card us-top6"><div class="section-title"><div><div class="eyebrow">LIVE RANKING</div><h3>R5.1 Top 6</h3></div></div>'+ranks+'</div></div></section>'+
...
def main():
    ...
    app=once(app,"if(m==='US'){const a=(j.payload?.top3||[]).slice(0,3);bindPerformanceTabs('US',a,j);loadYtdStockCharts('US',a)}","if(m==='US'){const a=(j.payload?.assets||j.payload?.top3||[]).slice(0,6);bindUsWorkspace();bindPerformanceTabs('US',a,j);loadYtdStockCharts('US',a.slice(0,3));renderCommandModel(j)}",'US binding')
    ...
    for m in ("commandAccount","commandModel","commandHealth","R5.1 Top 6","usLedgerTable","usModelTable","bindUsWorkspace"):
