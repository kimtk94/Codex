...
function sessionStatusHtml(){
  ...
  return '<section id="us-session-status" class="sessioncard"><div class="sessiontop"><div><span class="eyebrow">US SESSION · KST DATE</span><b>'+E(x.kstSessionDate)+' · US Session</b><small>NY '+E(x.usDate)+' · KST '+E(fmtKstMD(x.slots[0]))+' 밤 → '+E(fmtKstMD(x.slots.at(-1)))+' 새벽 · Automation 23:35 → 04:35</small></div><span class="sessionstate '+x.statusClass+'">'+E(x.status)+'</span></div><div class="sessionprogress"><div style="width:'+pct+'%"></div></div><div class="sessionmeta"><b>'+x.done+'/'+x.slots.length+' cycles reflected</b><span>Latest web update '+E(S?.generated_at?(fmtKstMD(new Date(S.generated_at))+' '+fmtKstHM(new Date(S.generated_at))+' KST'):'—')+'</span></div><div class="sessionsteps">'+steps+'</div><div class="nextgrid"><div><span>NEXT ACTION</span><b>'+E(fmtKstMD(x.next))+' '+E(fmtKstHM(x.next))+' KST</b><small>'+(sameSession?'Next US serialized cycle':'Next US trading-day cycle')+'</small></div><div><span>NEXT UPDATE</span><b>~ '+E(fmtKstMD(x.nextUpdate))+' '+E(fmtKstHM(x.nextUpdate))+' KST</b><small>expected after cycle completion · observed lag based</small></div></div></section>';
}
