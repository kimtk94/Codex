...
SCHEMA = "kalman-r7-1-actual-session-reaction-v1"
COST = 0.001
MIN_UNIVERSE = 80
BLOCK_DAYS = 5
B_DEFAULT = 2000
...
CHALLENGER = "R7C1_ACTUAL_SESSION_REACTION_ONLY"

INDICATOR_SCALE = {
    "CPI_HEADLINE_MOM": (0.1, 1.0),
    "CORE_CPI_MOM": (0.1, 1.0),
    "NFP": (50.0, 1.0),  # thousands
    "UNEMPLOYMENT_RATE": (0.1, -1.0),
    "AVERAGE_HOURLY_EARNINGS_MOM": (0.1, 1.0),
    "PPI_MOM": (0.2, 1.0),
    "JOLTS_OPENINGS": (250.0, 1.0),  # thousands
}
FAMILY_FEATURE = {
    "CPI": "m_cpi_decay",
    "EMPLOYMENT": "m_employment_decay",
    "PPI": "m_ppi_decay",
    "JOLTS": "m_jolts_decay",
    "FOMC": "m_fomc_decay",
}
...
def parse_args():
    ...
    p.add_argument("--events", default="/opt/kalman/state/r7_macro_free/events.json")
    p.add_argument("--actual-manifest", default="/opt/kalman/state/r7_macro_free/manifest.json")
    p.add_argument("--reactions", default="/opt/kalman/state/r7_macro_reaction/session_reactions.json")
    p.add_argument("--reaction-manifest", default="/opt/kalman/state/r7_macro_reaction/manifest.json")
...
def build_event_states(events_path: Path, reactions_path: Path):
    events = json.loads(events_path.read_text(encoding="utf-8"))
    reactions = json.loads(reactions_path.read_text(encoding="utf-8"))
    reaction_by_key = {
        (str(r.get("family")), str(r.get("release_at"))): r for r in reactions
    }

    previous = {}
    states = []
    for e in sorted(events, key=lambda x: str(x.get("release_at") or "")):
        family = str(e.get("family") or "")
        release_at = nts(e.get("release_at"))
        if pd.isna(release_at):
            continue

        delta_scores = []
        actuals = e.get("first_release_actuals") or {}
        if bool(e.get("pit_actual_eligible")):
            for indicator, value in actuals.items():
                if indicator not in INDICATOR_SCALE or value is None:
                    continue
                try:
                    current = float(value)
                ...
                if indicator in previous:
                    scale, sign = INDICATOR_SCALE[indicator]
                    z = sign * (current - previous[indicator]) / scale
                    delta_scores.append(float(np.clip(z, -5.0, 5.0)))
                previous[indicator] = current

        actual_delta_pressure = float(np.mean(delta_scores)) if delta_scores else np.nan

        r = reaction_by_key.get((family, release_at.isoformat()))
        if r is None:
            # JSON may preserve +00:00 identically, but keep a tolerant fallback.
            r = next(
                (x for x in reactions
                 if str(x.get("family")) == family
                 and nts(x.get("release_at")) == release_at),
                None,
            )
        if not r or not bool(r.get("reaction_available")):
            continue

        available_at = nts(r.get("reaction_available_at"))
        if pd.isna(available_at):
            continue
        reaction = pd.to_numeric(r.get("qqq_reaction_1h_open_to_close"), errors="coerce")
        states.append({
            "release_at": release_at,
            "available_at": available_at,
            "family": family,
            "actual_delta_pressure": actual_delta_pressure,
            "session_reaction_1h": float(reaction) if pd.notna(reaction) else np.nan,
        })

    s = pd.DataFrame(states).sort_values("available_at").reset_index(drop=True)
    if s.empty:
        raise RuntimeError("no PIT event states available")
    return s


def attach_macro_features(df: pd.DataFrame, states: pd.DataFrame):
    out = df.copy()
    ...
    left = (
        out[["signal_as_of"]]
        .drop_duplicates()
        .sort_values("signal_as_of")
        .reset_index(drop=True)
    )
    joined = pd.merge_asof(
        left,
        states.sort_values("available_at"),
        left_on="signal_as_of",
        right_on="available_at",
        direction="backward",
        allow_exact_matches=True,
    )

    age_h = (joined["signal_as_of"] - joined["release_at"]).dt.total_seconds() / 3600.0
    active = age_h.between(0, 72, inclusive="both")
    decay = np.where(active, np.exp(-math.log(2.0) * age_h / 24.0), 0.0)

    joined["m_event_decay_24h"] = decay
    joined["m_actual_delta_pressure_decay"] = (
        pd.to_numeric(joined["actual_delta_pressure"], errors="coerce") * decay
    ).where(active, 0.0)
    joined["m_session_reaction_1h_decay"] = (
        pd.to_numeric(joined["session_reaction_1h"], errors="coerce") * decay
    ).where(active, 0.0)

    for family, col in FAMILY_FEATURE.items():
        joined[col] = np.where(active & (joined["family"] == family), decay, 0.0)

    feat = joined[["signal_as_of", *MACRO_FEATURES]].copy()
    out = out.merge(feat, on="signal_as_of", how="left", validate="many_to_one")
    for col in MACRO_FEATURES:
        out[col] = pd.to_numeric(out[col], errors="coerce").fillna(0.0)

    audit = {
        "rows": int(len(out)),
        "timestamps": int(out["timestamp"].nunique()),
        "timestamps_with_active_macro": int(
            out.loc[out["m_event_decay_24h"] > 0, "timestamp"].nunique()
        ),
        "active_timestamp_ratio": float(
            (out.groupby("timestamp")["m_event_decay_24h"].first() > 0).mean()
        ),
        "max_decay": float(out["m_event_decay_24h"].max()),
...
def main():
    ...
    out = us / "model_lab_v1/results/r7_1_actual_session_reaction"
    out.mkdir(parents=True, exist_ok=True)

    if os.getenv("R7_ALLOW_LIVE", "").lower() == "true":
        raise RuntimeError("R7.1 refuses LIVE mode")

    actual_manifest = json.loads(Path(a.actual_manifest).read_text())
    reaction_manifest = json.loads(Path(a.reaction_manifest).read_text())
    if not actual_manifest.get("pit_actual_ready"):
        raise RuntimeError("R7 actual PIT gate not ready")
    if not reaction_manifest.get("session_reaction_ready"):
        raise RuntimeError("R7 session reaction gate not ready")
    if reaction_manifest.get("reaction_contract") != "QQQ_SESSION_REACTION_1H":
        raise RuntimeError("unexpected reaction contract")
    ...
    states = build_event_states(Path(a.events), Path(a.reactions))
    train_all, train_macro_audit = attach_macro_features(train_all, states)
    scored, scored_macro_audit = attach_macro_features(scored, states)

    # Sanity: no event feature may precede its reaction availability.
    if scored["m_event_decay_24h"].max() <= 0:
        raise RuntimeError("macro features are entirely inactive in scored rows")
    ...
    for fold, ss, ee in EVAL_FOLDS:
        ...
        fold_contract.append({
            ...
            "active_macro_train_timestamp_ratio": float(
                (tr.groupby("timestamp")["m_event_decay_24h"].first() > 0).mean()
            ),
            "active_macro_test_timestamp_ratio": float(
                (te.groupby("timestamp")["m_event_decay_24h"].first() > 0).mean()
            ),
        })
    ...
    decision = {
        ...
        "macro_feature_contract": {
            "features": MACRO_FEATURES,
            "event_state": "latest reaction-available event only",
            "half_life_hours": 24,
            "max_age_hours": 72,
            "actual_component": "change vs prior PIT-eligible first-release actual, fixed scale/policy sign, clipped +/-5",
            "reaction_component": "QQQ first complete 60m post-event open-to-close return",
            "consensus_used": False,
            "intraday_us2y_used": False,
        },
        "readiness": {
            "pit_actual_ready": bool(actual_manifest.get("pit_actual_ready")),
            "pit_actual_coverage": actual_manifest.get("pit_archive_coverage_ratio"),
            "session_reaction_ready": bool(reaction_manifest.get("session_reaction_ready")),
    ...
    eval_keep = [
        ...
        CONTROL, CHALLENGER, *MACRO_FEATURES,
    ]
    eval_rows[eval_keep].to_parquet(out / "r7_1_scored_rows.parquet", index=False)
    challenger.to_parquet(out / "r7_1_trade_ledger.parquet", index=False)
    leaderboard.to_csv(out / "r7_1_leaderboard.csv", index=False)
    pd.DataFrame(fold_contract).to_csv(out / "r7_1_fold_contract.csv", index=False)
    pd.DataFrame(boot["folds"]).to_csv(out / "r7_1_paired_fold_summary.csv", index=False)
    (out / "r7_1_selection_decision.json").write_text(
        json.dumps(decision, indent=2, ensure_ascii=False, default=str) + "\n"
    )
    (out / "r7_1_manifest.json").write_text(
        json.dumps({
            "schema": SCHEMA,
            ...
            "sources": {
                ...
                str(a.reactions): sha256_file(Path(a.reactions)),
            },
            "research_only": True,
            "production_changed": False,
        }, indent=2) + "\n"
