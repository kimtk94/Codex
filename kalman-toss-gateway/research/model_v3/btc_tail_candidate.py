...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Freeze or track Kalman V3.003 BTC tail-entry candidate"
    )
...
def select_tail_candidate(
    ...
    *,
    ...
    market_eval = evaluation_spec["markets"]["BTC"]
    gate = model_spec["selection_gate"]
    minimum_oof = int(model_spec["walk_forward"]["minimum_oof_rows"])
    minimum_entries = max(
        int(gate["minimum_nonoverlap_entries"]),
        int(evaluation_spec["policy_selection"]["minimum_nonoverlap_entries"]),
    )
        ...
        for fraction in market_eval["candidate_top_fractions"]:
            ...
            checks = {
                ...
                "positive_incremental_alpha_net": (
                    ...
                    and float(
                        strategy["nonoverlap_incremental_alpha_mean_net"]
                    ) > 0.0
                ),
    ...
    joint = [x for x in rows if x["joint_gate_pass"]]
    ...
    audit = {
        ...
        "candidate_policy_results": rows,
        "joint_gate_passing_top": joint[:12],
    }
        ...
        audit.update(
            {
                ...
                "next_action": "REDESIGN_BTC_TAIL_MODEL_FAMILY",
            }
        )
    ...
    best_oof = oof_by_candidate[best_id].copy()
    ...
    policy = LockedQuantilePolicy(
        ...
        source="EXPANDING_WALK_FORWARD_OOF_BTC_TAIL_PRE_FORWARD_ONLY",
    )
    audit.update(
        {
            ...
            "next_action": "FREEZE_BTC_TAIL_FORWARD_CANDIDATE",
        }
    )
    return hp, policy, audit, best_oof


def freeze_btc_model(
    train: pd.DataFrame,
    *,
    ...
    return {
        ...
        "objective": "TAIL_ENTRY_ALPHA",
        "score_semantics": "RANK_SCORE_NOT_CALIBRATED_PROBABILITY",
...
def build_reference_with_bridge(
    *,
    ...
    if reference.empty:
        raise RuntimeError("BTC tail policy reference is empty")
    if pd.Timestamp(reference["as_of"].max()) >= forward_start:
        raise RuntimeError("BTC tail reference crosses forward boundary")
...
def main() -> int:
    ...
    horizon = int(model_spec["markets"]["BTC"]["horizon_observations"])
    if int(evaluation_spec["markets"]["BTC"]["horizon_observations"]) != horizon:
        raise RuntimeError("model/evaluation BTC horizon mismatch")
    ...
    if model_path.exists():
        ...
        if artifact.get("artifact_schema") != ARTIFACT_SCHEMA:
            raise RuntimeError("existing BTC artifact schema mismatch")
        if artifact.get("model_version") != model_spec["version"]:
            raise RuntimeError("existing BTC model version mismatch")
        if artifact.get("frozen") is not True:
            raise RuntimeError("existing BTC artifact is not frozen")
        for required in [reference_path, selection_path, reference_audit_path]:
            if not required.exists():
                raise RuntimeError(f"missing frozen BTC file: {required}")
        ...
        hp, policy, selection_audit, selected_oof = select_tail_candidate(
            oof_by_candidate,
        ...
        if hp is None or policy is None or selected_oof is None:
            report = {
                "schema_version": "kalman-v3-003-btc-tail-report-v1",
                "status": "NO_JOINT_GATE_CANDIDATE",
                "mode": "PRE_FORWARD_OOF_AUDIT_ONLY",
                "freeze_created": False,
                "model_version": model_spec["version"],
                "strategy_version": model_spec["markets"]["BTC"][
                    "strategy_version"
                ],
        ...
        freeze_created = True
        mode = "FREEZE_NEW_BTC_TAIL_MODEL"
    ...
    if pd.Timestamp(frozen_reference["as_of"].max()) >= forward_start:
        raise RuntimeError("BTC frozen reference contains forward rows")
