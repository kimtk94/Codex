...
if [ -z "${KOGES_INPUT:-}" ]; then
  for CANDIDATE in     "$ROOT/data/metabolic_resilience/stage0_koges/public_training"     "$ROOT/data/metabolic_resilience/stage0/public_training"     "$ROOT/data/ckd/stage4_koges/public_training"     "$ROOT/results/metabolic_resilience/stage0_koges/public_training"
  do
    if [ -d "$CANDIDATE" ]; then
...
for STAGE in stage0 stage1 stage1b stage2 stage2b stage3 stage4 stage4b stage5 stage6 stage7
do
...
RC2B=0
if [ "$RUN_SENSITIVITY" = "1" ] && [ "$RC1" -eq 0 ]; then
...
printf "FINAL stage0=%s stage1=%s stage1b=%s stage2=%s stage2b=%s stage3=%s stage4=%s stage4b=%s stage7=%s\n"   "$RC0" "$RC1" "$RC1B" "$RC2" "$RC2B" "$RC3" "$RC4" "$RC4B" "$RC7"
