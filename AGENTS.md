...
추가 규칙:
- LC-MS 문의의 기준 문서는 `submission_LCMS.md` 1개로 통일한다.
- `service_basics/lc_ms_submission.md`는 중복 요약을 두지 않고, LC-MS 문의 분기/확인 체크만 유지한다.
...
### 8. Olink 견적 회신 추가 규칙
- Olink Explore/Reveal 견적은 서비스 단가에 시퀀싱이 포함된 구조이므로, 고객 발송 본문에서 시퀀싱 항목을 별도로 분리/가산하지 않는다.
- `Singapore 수행 조건`은 내부 확인/메모 용도로만 관리하고, 고객 발송용 본문에는 기본적으로 기재하지 않는다.
- 최소 샘플 수는 Olink Reveal, Olink Explore 모두 24 samples 기준으로 안내한다.
...
| LC-MS Sample Submission | `submission_LCMS.md` (LC-MS 문의 시에만 확인) |
